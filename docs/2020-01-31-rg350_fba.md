...
9. Al volver a la lista de ROMs (esta vez no hace falta provocar el rescaning) veremos solo las ROMs que podemos ejecutar:
...
1. Partimos del directorio con las imágenes en nuestro ordenador. Renombrarlo como `previews`.
2. Extraer la tarjeta externa de la consola, montarla en el ordenador y copiar el directorio `previews` a la raíz de la misma.
...
5.  Como seguramente el directorio ya existirá, se nos pedirá confirmación para sobreescribirlo. Seleccionar `Yes to all` y confirmar con `A`:
