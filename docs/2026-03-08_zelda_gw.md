...
A continuación limpiamos con malla desoldadora la huella del chip que acabamos de retirar.
...
Ahora toca compilar el firmware (si no vamos a utilizar portadillas para aprovechar el espacio de sus ficheros para poder poner más ROMs, no incluir las variables de entorno `COVERFLOW` y `JPG_QUALITY` en el comando de compilación):
...
Manteniendo pulsado el botón `PAUSE/SET` mientras se presionan otros botones se realizan las siguientes acciones:
