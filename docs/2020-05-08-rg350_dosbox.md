...
Un parámetro interesante del `dosbox.conf` es el `scaler` que se encuentra dentro de la sección `[render]`. Permite seleccionar de entre una serie de filtros para producir efectos en la pantalla (como los típicos scanlines de las teles CRT). Por ejemplo en el dosbox.conf predeterminado se ve esto:
