...
The original firmware serves to verify the solvency of the platform to emulate at least the 8-bit machines that originally run, so the device immediately became attractive to the hacker community. Shortly after it went on sale, procedures appeared to modify the machine so that more emulators and games could be installed.
...
In this article we will describe the steps to perform these two modifications. The modification to add the microSD slot also requires modifying the flash, since every time we load a new game from the microSD it will be written to the flash chip, and the original 4MB one supports few write cycles since it was not intended to be used this way.
...
Next, we clean the footprint of the chip we just removed with desoldering braid.
...
Now it's time to compile the firmware (if we are not going to use covers to take advantage of the space of their files to be able to put more ROMs, do not include the `COVERFLOW` and `JPG_QUALITY` environment variables in the compilation command):
...
When compilation finishes, we will be informed of the resulting binary size that will be flashed to the chip. For example:
...
The savestates will appear in the `~/git/game-and-watch-retro-go/save_states` subdirectory. Once we have compiled and flashed a new firmware, we can restore them by running the restoration script:
...
If we turn on the console at that moment, after removing the programming probe, we will see that the bootloader starts and shows us the following warning:
...
Holding down the `PAUSE/SET` button while pressing other buttons performs the following actions:
