...
Si por ejemplo queremos añadir un sistema nuevo, tan solo tendremos que crear un nuevo directorio con un fichero `theme.xml` y resto de ficheros a los que luego se haga referencia en él. Por ejemplo este es el contenido del directorio correspondiente a Game Boy en el tema `Simple`:
