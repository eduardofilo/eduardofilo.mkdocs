...
* Ports y Emuladores de Gameblabla:
    ...
    * [En su sitio](https://gameblabla.nl/files/ipk/gcw0/)
...
* Mods de emuladores:
    * [ReGBA 1.45.5 sin bezel](/files/pages/regba-1.45.5.opk)
    * [FBA con doble launcher (UX y Explorer)](/files/pages/fba.opk)
    * [MAME4All con doble launcher (UX y Explorer)](https://github.com/goldmojo/mame4all/releases/download/Mame4All-params-v5/mame4all-params-5-skipwarnings.opk)
    * [DOSBox que permite personalizar el `dosbox.conf` en cada juego incluyéndolo en su directorio](/files/posts/dosbox/Dosbox_conf_mod.opk). Descripción más detallada [aquí](/2020-05-08-rg350_dosbox.html#personalizacion-de-controles-para-juegos)
...
* Linux kernel:
    * [GCW0](https://github.com/gcwnow/linux)
...
* Frontends:
    ...
    * [EmulationStation](https://github.com/ManuelSch81/RG350-EmulationStation_configured): [Monográfico](/2020-02-13-rg350_emulationstation.html)
...
* Repuestos componentes:
    ...
    * [Protector de pantalla](https://www.amazon.es/gp/product/B07X5NZ85F)
    ...
    * [Conductice Rubber Pad (D-Pad)](https://www.ebay.com/itm/New-RG300-RG350-Retro-Mini-Retro-FC-Conductice-Rubber-Pad-D-Pad-2pcs-/164022916345)
...
En Windows un buen programa para transferir archivos por FTP a la consola es [WinSCP](https://winscp.net/eng/index.php). A partir del minuto 3:45 de [este vídeo](https://www.youtube.com/watch?v=V7bWAM0xyzY&t=225) se ve cómo se usa.
...
Entre los assents de cada [release](https://github.com/Ninoh-FOX/RG350-ROGUE-CFW/releases) encontramos tres opciones:

1. **Update**: Por ejemplo en la release 1.7.9.9 el fichero sería: [rg350-update-1.7.9.9-2020-04-25.opk](https://github.com/Ninoh-FOX/RG350-ROGUE-CFW/releases/download/1.7.9.9/rg350-update-1.7.9.9-2020-04-25.opk). Es una aplicación OpenDingux, por tanto se ejecuta desde la propia consola. No formatea, sino que sustituye los archivos que han cambiado en el nuevo firmware respecto de un sistema anterior. Recomendable para no perder ajustes de usuario, aunque dependiendo de la versión del sistema de partida puede no quedar del todo fino. Se recomienda utilizar esta opción sólo para actualizar entre versiones de ROGUE, es decir si se parte de BASE, hay que utilizar una de las dos siguientes (preferentemente la última).
2. **Flasher**: Por ejemplo en la release 1.7.9.9 el fichero sería: [rg350-flasher-1.7.9.9-2020-04-26.opk](https://github.com/Ninoh-FOX/RG350-ROGUE-CFW/releases/download/1.7.9.9/rg350-flasher-1.7.9.9-2020-04-26.opk). Es una aplicación OpenDingux, por tanto se ejecuta desde la propia consola. Formatea completamente la SD interna con el nuevo firmware por lo que se pierden ajustes del usuario. Recomendable para solucionar problemas en updates que no han quedado bien. En [este video](https://www.youtube.com/watch?v=V7bWAM0xyzY) se ilustra todo el procedimiento.
3. **sd_image.bin**: Por ejemplo en la release 1.7.9.9 el fichero sería: [sd_image.bin](https://github.com/Ninoh-FOX/RG350-ROGUE-CFW/releases/download/1.7.9.9/sd_image.bin). Es una imagen para flashear la tarjeta interna desde un ordenador. Es parecida a la opción 2 y la única que sirve si se parte de un sistema que no arranca. La imagen ocupa más porque es un volcado de todo lo que contiene la SD (incluidos los espacios vacíos de la tarjeta). El proceso incorpora la complejidad adicional de que tenemos que abrir la consola para extraer la microSD interna y utilizar algunas utilidades poco habituales. En [este video](https://www.youtube.com/watch?v=gyNDbmvH5Ro) (en inglés) se ilustra todo el procedimiento. La primera parte de [este otro vídeo](https://www.youtube.com/watch?v=VWJU3KR5MW4) también muestra el proceso, esta vez en español.
...
|`.linker`|Instala los links para GMenu2X de los programas preinstalados.|
|`.partition_resized`|Redimensiona la partición data de la SD interna para aprovechar todo el espacio disponible en la tarjeta.|
|`.reconfig`|Borra algunos ficheros residuales SSH y de otros programas.|
|`.swapon`|Crea el fichero para la memoria SWAP.|
...
|SimpleMenu| |[Listado](/2020-01-25-rg350_simplemenu.html#controles)|
|FBA| |[Listado](/2020-01-31-rg350_fba.html#controles)|
|ScummVM| |[Listado](/2020-04-14-rg350_scummvm.html#controles)|
|xMAME| |[Listado](/2020-04-15-rg350_xmame.html#controles)|
|Daphne| |[Listado](/2020-05-04-rg350_daphne.html#controles)|
|OpenBOR| |[Listado](/2020-05-06-rg350_openbor.html#controles)|
|MAME4All| |[Listado](/2020-05-14-rg350_mame4all.html#controles)|
...
* **Harry Potter y la Cámara secreta**: Este juego se queda atascado en el video introductorio. La solución es cambiar la opción `Core settings > Emulation core` de `rec` a `int`. El emulador es más lento pero más preciso y se puede superar ese punto. En ese momento grabar un savestate, volver a poner `rec` en `Core settings > Emulation core` y finalmente cargar el savestate. ([fuente](https://www.elotrolado.net/hilo_rg-350-game-kiddy-gdk350-350h-y-otras-alternativas-a-la-gcw-zero-con-el-jz4770_2341546_s1900#p1748409434))
* **Vagrant Story**: De forma parecida al anterior, hay que ejecutar la intro cambiando la opción `Core settings > Emulation core` de `rec` a `int`. Cuando el juego permita el control crear un savestate, volver a poner `rec` en `Core settings > Emulation core` y cargar el savestate. El `Analog Mode` que soporta este juego es `DualShock`. Recordar además que este juego es de los que necesita [SBI](http://psxdatacenter.com/sbifiles.html).
...
#### [Previews de ROMs en Gmenu2x](/2020-01-11-rg350_scraper.html)
