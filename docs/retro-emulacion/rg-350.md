...
|`/sys/devices/platform/13050000.lcd-controller/graphics/fb0/modes`|Permite conocer la resolución de la pantalla en OD Beta|
|`/sys/class/graphics/fb0/virtual_size`|Modo del framebuffer en OD Beta (?)|
|`/media/data/local/share/opendingux/loading.png`|Pantalla mostrada durante las transiciones de modo en OD Beta|
|`~/.autostart`|Script para lanzar el frontend en OD Beta. Si no existe se lanza GMenu2X por defecto|
|`/sys/kernel/debug/gpio`|Estado de los GPIOs del procesador y correspondencia con controles en OD Beta|
