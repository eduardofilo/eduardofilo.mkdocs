...
Las ROMs son los volcados a fichero del soporte digital donde se almacenaba el código y los datos (sonidos por ejemplo) del juego en la máquina original. En viejas recreativas y en máquinas de cartucho, este soporte solían ser chips de memoria no volátil. En las máquinas más modernas normalmente son CD's, DVD's, Bluray's. Las ROMs en la RG350 se suelen colocar en la tarjeta microSD externa. Pueden ir en la interna pero puede complicar el proceso de actualización del firmware. En cuanto a la estructura de carpetas, puede ser cualquiera ya que los emuladores normalmente al abrirlos, presentan una especie de explorador de archivos para seleccionar la ruta y archivo de la ROM a ejecutar. Aun así más o menos todo el mundo utiliza una estructura similar en la que los distintos sistemas se meten dentro de una carpeta llamada `roms` en la raíz de la tarjeta. Cadda sistema a su vez suele ser otra carpeta con un nombre acrónimo de la máquina. Más o menos se utilizan los siguientes acrónomos:
...
2. **Flasher**: Es una aplicación OpenDingux, por tanto se ejecuta desde la propia consola. Formatea completamente la SD interna con el nuevo firmware por lo que se pierden ajustes del usuario. Recomendable para solucionar problemas en flasheos que no han quedado bien.
3. **sd_image.bin**: Es una imagen para flashear la tarjeta interna desde un ordenador. Es parecida a la opción 1 y la única que sirve si se empieza con un sistema que no arranca. La imagen ocupa más porque es un volcado de todo lo que contiene la SD (incluidos los espacios vacíos de la tarjeta).
...
Utilizar [Balena Etcher](https://www.balena.io/etcher/).
...
En ocasiones no se muestra correctamente el icono de una aplicación. Puede suceder cuando hay dos OPK's con el mismo nombre en la SD interna y externa. Para resetear la información, entrar en la sección donde esté el problema dentro de el directorio `/usr/local/home/.gmenu2x/sections` y borrar el nombre de la aplicación problemática. Lo que hay allí no es la aplicación en sí sino un fichero tipo `.desktop` con metainformación (icono, descripción, etc.).
