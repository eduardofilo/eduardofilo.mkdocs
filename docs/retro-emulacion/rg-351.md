...
Por defecto la paleta de colores que utiliza Gambatte, el emulador de GB/GBC, no es muy afortunada. La imagen tiene un tono amarillo poco atractivo o parecido a los tonos de la pantalla de la Game Boy. Se recomienda hacer los siguientes ajustes:

1. `Quick Menu > Options > GB Colorization`: `Internal`
2. `Quick Menu > Options > Internal Palette`: `GB - DMG`
3. `Main Menu > Settings > Video > Bilinear Filtering`: `OFF`
4. `Main Menu > Settings > Video > RGA Scaling`: `OFF`
5. `Quick Menu > Overrides > Save Core Overrides`: Para convertir los ajustes anteriores en predeterminados para el core de GB/GBC.
