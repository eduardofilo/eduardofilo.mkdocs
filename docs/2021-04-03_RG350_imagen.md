...
Se comentan aquí los detalles de una imagen de tarjeta interna preparada para las consolas RG350, RG350P, RG350M, RG280V y RG280M. La imagen es un volcado de una microSD de 4GB con sistema ROGUE (agradecimiento especial a [Ninoh-FOX](https://github.com/Ninoh-FOX) por su soporte), ports, emuladores y cores RetroArch y los frontends GMenu2X, SimpleMenu (agradecimiento especial a [FGL82](https://github.com/fgl82) por su soporte) y PyMenu. Se puede flashear sobre una tarjeta de ese tamaño o superior. Durante el primer arranque se expandirá el tamaño de la partición principal hasta ocupar todo el espacio disponible en la tarjeta.

Descarga de la imagen desde los siguientes enlaces:

* [Panda](https://drive.google.com/file/d/1QrMYXZXGm8idTy7ceixC746V4xFECnkW/view?usp=sharing) (RG350 y RG350P)
* [Mapache](https://drive.google.com/file/d/15mvH5UbVgQcAlkcaKZhgbo3jPB_MwYul/view?usp=sharing) (RG350M)
* [Visón](https://drive.google.com/file/d/18cFQvN4C0iRBk6eqtIAywLdO2VKjx7AO/view?usp=sharing) (RG280V)
* [Musaraña](https://drive.google.com/file/d/1kaQXQYIsM7oxmE_0rNCJz5omqWQLV8It/view?usp=sharing) (RG280M)
...
Los ports o juegos instalados son los siguientes (algunos como `Cannonball` y `OpenJazz` necesitan ROMs en la tarjeta externa):
...
$ gunzip panda_v10.0.img.gz -c | sudo dd of=/dev/mmcblk0 bs=2M status=progress conv=fsync
```
...
|OpenBOR/Paks      |OpenBOR             |.pak|

**Advertencia**: Ojo al caso de OpenBOR que se comporta de forma irregular al exigir una ruta fija en la tarjeta externa.

En el caso de los sistemas para los que exista core RetroArch, también servirán las extensiones `zip` y `7z`, ya que se soportan a nivel de framework. También se soporta la extensión `chd` en los cores RetroArch de `SEGACD` y `PCECD`.
