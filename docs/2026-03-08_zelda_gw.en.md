...
The entire process has been performed on a Linux system. In principle, development on Windows and MacOS X systems is also possible, but the steps to install the requirements and the software we will use will necessarily change.
...
There is an alternative if we don't want to risk permanently damaging the original cover, and that is to 3D print the model created by the user Aradia, which can be downloaded [here](files/posts/2026-03-08_zelda_gw/G&W%20Zelda%20-%20Coque%20arrière%20-%20Mod%20Retro%20Go%20SD%20-%20v1.stl).
...
| `PAUSE/SET` + `POWER`        | Poweroff without savestate                                             |
...
| `PAUSE/SET` + `GAME` + `LEFT` | From the OFW, it causes the bootloader to start without fully loading retro-go. It is used to see the screen that shows the SD detection, flash chip, and code in bank2 of the internal flash; it is also used to see the bootloader version. |
| `GAME` + menu `Reset settings` | Clears the registration data in the SD of what is cached in the flash; use when changing the SD of the machine or when changing the flash chip. It is equivalent to deleting the file `flashcachedata.bin` in the `data` directory of the SD. |
